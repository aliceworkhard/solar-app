...
const RESULTS_PATH = path.join(ROOT, ".agent/reports/2026-04-29-t031-final-layout-results.json");
...
function runStaticChecks() {
  const mainActivity = fs.readFileSync(
    path.join(APP_DIR, "android/app/src/main/java/com/solar/remote/MainActivity.java"),
    "utf8"
  );
  ...
  if (!/EDGE_PROBE_COLORS\s*=\s*false/.test(mainActivity)) {
    throw new Error("EDGE_PROBE_COLORS must be false for final sideload");
  }
  if (/EDGE_PROBE_COLORS\s*=\s*true/.test(mainActivity)) {
    throw new Error("EDGE_PROBE_COLORS=true leaked into final sideload");
  }
  if (!mainActivity.includes("TRANSPARENT_EDGE_WITH_STRIPS")) {
    throw new Error("Final edge strategy should use TRANSPARENT_EDGE_WITH_STRIPS");
  }
  if (/Color\.(CYAN|GREEN|MAGENTA|YELLOW)/.test(mainActivity)) {
    throw new Error("Named probe colors leaked into MainActivity final path");
  }
  if (/edge-probe|edge-diagnostic|lime|#00ffff|#00ff00|#ff00ff|#ffff00/i.test(appTs + styles)) {
    throw new Error("Web/CSS final path contains probe class or probe color tokens");
  }
}
...
async function main() {
  ...
  const chromeUserDataDir = path.join(process.env.TEMP || APP_DIR, `solar-remote-t031-final-chrome-${Date.now()}`);
  ...
  try {
    ...
    for (const width of [320, 360, 390]) {
      ...
      const evaluation = await cdp.send("Runtime.evaluate", {
        ...
        expression: `(() => {
          ...
          return JSON.stringify({
            ...
            datasetEdgeBuild: document.documentElement.dataset.edgeBuild,
            bodyClasses: Array.from(document.body.classList).sort(),
            overlayBackground: docStyle.getPropertyValue("--edge-dom-overlay-background").trim(),
            ...
            bottomNavBackground: navStyle.backgroundColor,
            bottomNavPaddingBottom: navStyle.paddingBottom,
      ...
      if (result.edgeBuildId !== "T031-system-bars-final" || result.datasetEdgeBuild !== "T031-system-bars-final") {
        throw new Error(`width ${width} missing T031 build id`);
      }
      if (!result.bodyClasses.includes("edge-transparent")) {
        throw new Error(`width ${width} final transparent edge class missing: ${result.bodyClasses.join(" ")}`);
      }
      if (result.bodyClasses.includes("edge-visual-fallback") || result.bodyClasses.includes("edge-color-match")) {
        throw new Error(`width ${width} unexpected edge mode class: ${result.bodyClasses.join(" ")}`);
      }
      if (result.overlayBackground !== "transparent") {
        throw new Error(`width ${width} diagnostic overlay visible: ${result.overlayBackground}`);
      }
      if (!result.shellBackgroundImage.includes("242, 248, 255") || !result.appBackgroundImage.includes("232, 244, 255")) {
        throw new Error(`width ${width} page background is not the final light gradient`);
      }
      if (result.bottomNavBackground !== "rgba(228, 242, 255, 0.96)") {
        throw new Error(`width ${width} bottom nav background mismatch: ${result.bottomNavBackground}`);
      }
      ...
      if (result.shellPaddingLeft !== expectedPadding || result.shellPaddingRight !== expectedPadding) {
        throw new Error(`width ${width} shell padding compressed: ${result.shellPaddingLeft}/${result.shellPaddingRight}`);
      }
      ...
      if (result.headerTitleTop < 10) {
        throw new Error(`width ${width} header title too high: ${result.headerTitleTop}`);
      }
      if (Math.abs(result.navBottom - result.height) > 1) {
        throw new Error(`width ${width} bottom nav not attached to viewport bottom`);
      }
      if (result.activeItemBottom > result.height - 18) {
        throw new Error(`width ${width} active tab overlaps final bottom inset: ${result.activeItemBottom}`);
      }
    }
    ...
    fs.writeFileSync(
      path.join(SCREENSHOT_DIR, "2026-04-29-t031-final-390x900.png"),
