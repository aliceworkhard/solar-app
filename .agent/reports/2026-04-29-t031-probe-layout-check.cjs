const fs = require("fs");
const http = require("http");
const path = require("path");
const { spawn } = require("child_process");

const ROOT = "C:/Users/SJGK8/Desktop/PROJECT/太阳能遥控器app";
const APP_DIR = path.join(ROOT, "项目文件/android-mvp-capacitor");
const SCREENSHOT_DIR = path.join(ROOT, ".agent/reports/screenshots");
const RESULTS_PATH = path.join(ROOT, ".agent/reports/2026-04-29-t031-probe-layout-results.json");
const CHROME_PATH = process.env.CHROME_PATH || "C:/Program Files/Google/Chrome/Application/chrome.exe";
const APP_PORT = 5180;
const DEBUG_PORT = 9230;
const APP_URL = `http://127.0.0.1:${APP_PORT}`;

fs.mkdirSync(SCREENSHOT_DIR, { recursive: true });

function wait(ms) {
  return new Promise((resolve) => setTimeout(resolve, ms));
}

function requestJson(url) {
  return new Promise((resolve, reject) => {
    const req = http.get(url, (res) => {
      let data = "";
      res.setEncoding("utf8");
      res.on("data", (chunk) => {
        data += chunk;
      });
      res.on("end", () => {
        if (res.statusCode < 200 || res.statusCode >= 300) {
          reject(new Error(`${url} -> ${res.statusCode}: ${data}`));
          return;
        }
        resolve(JSON.parse(data));
      });
    });
    req.on("error", reject);
    req.setTimeout(3000, () => req.destroy(new Error(`Timeout requesting ${url}`)));
  });
}

function requestOk(url) {
  return new Promise((resolve, reject) => {
    const req = http.get(url, (res) => {
      res.resume();
      res.on("end", () => {
        if (res.statusCode < 200 || res.statusCode >= 400) {
          reject(new Error(`${url} -> ${res.statusCode}`));
          return;
        }
        resolve(true);
      });
    });
    req.on("error", reject);
    req.setTimeout(3000, () => req.destroy(new Error(`Timeout requesting ${url}`)));
  });
}

async function waitFor(check, label) {
  const deadline = Date.now() + 30000;
  let lastError;
  while (Date.now() < deadline) {
    try {
      return await check();
    } catch (error) {
      lastError = error;
      await wait(300);
    }
  }
  throw new Error(`Timed out waiting for ${label}: ${lastError?.message || "unknown"}`);
}

function startProcess(file, args, options = {}) {
  const command = file.endsWith(".cmd") ? "cmd.exe" : file;
  const commandArgs = file.endsWith(".cmd") ? ["/d", "/s", "/c", file, ...args] : args;
  return spawn(command, commandArgs, {
    cwd: options.cwd,
    stdio: "ignore",
    windowsHide: true
  });
}

function connectCdp(wsUrl) {
  const socket = new WebSocket(wsUrl);
  let nextId = 1;
  const pending = new Map();

  socket.addEventListener("message", (event) => {
    const message = JSON.parse(event.data);
    if (!message.id) {
      return;
    }
    const request = pending.get(message.id);
    if (!request) {
      return;
    }
    pending.delete(message.id);
    if (message.error) {
      request.reject(new Error(`${message.error.message}: ${message.error.data || ""}`));
      return;
    }
    request.resolve(message.result);
  });

  const opened = new Promise((resolve, reject) => {
    socket.addEventListener("open", resolve, { once: true });
    socket.addEventListener("error", reject, { once: true });
  });

  return {
    async send(method, params = {}) {
      await opened;
      const id = nextId++;
      socket.send(JSON.stringify({ id, method, params }));
      return new Promise((resolve, reject) => {
        pending.set(id, { resolve, reject });
      });
    },
    close() {
      socket.close();
    }
  };
}

function expectedShellPadding(width) {
  return width <= 360 ? 10 : width <= 430 ? 12 : 14;
}

async function main() {
  const results = [];
  const vite = startProcess("npm.cmd", ["run", "dev", "--", "--host", "127.0.0.1", "--port", String(APP_PORT)], {
    cwd: APP_DIR
  });
  const chromeUserDataDir = path.join(process.env.TEMP || APP_DIR, `solar-remote-t031-chrome-${Date.now()}`);
  const chrome = startProcess(CHROME_PATH, [
    `--remote-debugging-port=${DEBUG_PORT}`,
    `--user-data-dir=${chromeUserDataDir}`,
    "--headless=new",
    "--disable-gpu",
    "--hide-scrollbars",
    "about:blank"
  ]);

  try {
    await waitFor(() => requestOk(`${APP_URL}/`), "Vite dev server");
    const pages = await waitFor(() => requestJson(`http://127.0.0.1:${DEBUG_PORT}/json/list`), "Chrome DevTools");
    const pageInfo = pages.find((page) => page.type === "page");
    if (!pageInfo) {
      throw new Error("No Chrome page target found");
    }
    const cdp = connectCdp(pageInfo.webSocketDebuggerUrl);
    await cdp.send("Page.enable");
    await cdp.send("Runtime.enable");

    for (const width of [320, 360, 390]) {
      await cdp.send("Emulation.setDeviceMetricsOverride", {
        width,
        height: 900,
        deviceScaleFactor: 1,
        mobile: true
      });
      await cdp.send("Page.navigate", { url: APP_URL });
      await waitFor(
        async () => {
          const ready = await cdp.send("Runtime.evaluate", {
            returnByValue: true,
            expression: `Boolean(document.querySelector(".bottom-nav") && window.solarRemoteApplySystemInsets && window.__edgeBuildId)`
          });
          if (!ready.result.value) {
            throw new Error("App shell not ready");
          }
          return true;
        },
        `app shell ${width}`
      );

      const evaluation = await cdp.send("Runtime.evaluate", {
        returnByValue: true,
        expression: `(() => {
          window.solarRemoteApplySystemInsets?.({
            topPx: 96,
            rightPx: 54,
            bottomPx: 72,
            leftPx: 48,
            contentLeftPx: 0,
            contentRightPx: 0,
            density: 3,
            webViewWidthPx: window.innerWidth * 3,
            diagnosticColors: true
          });
          window.solarRemoteSetEdgeMode?.("visual-fallback");
          const docStyle = getComputedStyle(document.documentElement);
          const shellStyle = getComputedStyle(document.querySelector(".shell"));
          const headerTitle = document.querySelector(".app-header .page-title h1").getBoundingClientRect();
          const nav = document.querySelector(".bottom-nav").getBoundingClientRect();
          const activeItem = document.querySelector(".bottom-nav-item.active").getBoundingClientRect();
          return JSON.stringify({
            width: window.innerWidth,
            height: window.innerHeight,
            scrollWidth: document.documentElement.scrollWidth,
            edgeBuildId: window.__edgeBuildId,
            datasetEdgeBuild: document.documentElement.dataset.edgeBuild,
            bodyClasses: Array.from(document.body.classList).sort(),
            diagnosticBackground: docStyle.getPropertyValue("--edge-diagnostic-dom-background").trim(),
            nativeTop: docStyle.getPropertyValue("--native-inset-top").trim(),
            nativeBottom: docStyle.getPropertyValue("--native-inset-bottom").trim(),
            contentSafeLeft: docStyle.getPropertyValue("--content-safe-left").trim(),
            contentSafeRight: docStyle.getPropertyValue("--content-safe-right").trim(),
            shellPaddingLeft: shellStyle.paddingLeft,
            shellPaddingRight: shellStyle.paddingRight,
            headerTitleTop: headerTitle.top,
            navBottom: nav.bottom,
            activeItemBottom: activeItem.bottom
          });
        })();`
      });
      const result = JSON.parse(evaluation.result.value);
      results.push(result);

      const expectedPadding = `${expectedShellPadding(width)}px`;
      if (result.edgeBuildId !== "T031-system-bars-final" || result.datasetEdgeBuild !== "T031-system-bars-final") {
        throw new Error(`width ${width} missing T031 build id`);
      }
      if (!result.bodyClasses.includes("edge-visual-fallback")) {
        throw new Error(`width ${width} visual fallback class missing: ${result.bodyClasses.join(" ")}`);
      }
      if (result.bodyClasses.includes("edge-transparent") || result.bodyClasses.includes("edge-color-match")) {
        throw new Error(`width ${width} multiple edge mode classes: ${result.bodyClasses.join(" ")}`);
      }
      if (result.diagnosticBackground !== "rgba(90, 170, 255, 0.22)") {
        throw new Error(`width ${width} diagnostic DOM color mismatch: ${result.diagnosticBackground}`);
      }
      if (result.scrollWidth > width) {
        throw new Error(`width ${width} overflows: scrollWidth=${result.scrollWidth}`);
      }
      if (result.shellPaddingLeft !== expectedPadding || result.shellPaddingRight !== expectedPadding) {
        throw new Error(`width ${width} shell padding compressed: ${result.shellPaddingLeft}/${result.shellPaddingRight}`);
      }
      if (result.contentSafeLeft !== "0px" || result.contentSafeRight !== "0px") {
        throw new Error(`width ${width} content safe should ignore side gestures: ${result.contentSafeLeft}/${result.contentSafeRight}`);
      }
      if (result.headerTitleTop < 10) {
        throw new Error(`width ${width} header title too high: ${result.headerTitleTop}`);
      }
      if (Math.abs(result.navBottom - result.height) > 1) {
        throw new Error(`width ${width} bottom nav not attached to viewport bottom`);
      }
      if (result.activeItemBottom > result.height - 18) {
        throw new Error(`width ${width} active tab overlaps probe bottom inset: ${result.activeItemBottom}`);
      }
    }

    const screenshot = await cdp.send("Page.captureScreenshot", {
      format: "png",
      captureBeyondViewport: true
    });
    fs.writeFileSync(
      path.join(SCREENSHOT_DIR, "2026-04-29-t031-probe-390x900.png"),
      Buffer.from(screenshot.data, "base64")
    );

    fs.writeFileSync(RESULTS_PATH, JSON.stringify(results, null, 2));
    cdp.close();
  } finally {
    vite.kill();
    chrome.kill();
  }
}

main().catch((error) => {
  console.error(error);
  process.exit(1);
});
